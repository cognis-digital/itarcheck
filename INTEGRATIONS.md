...
**itarcheck** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `itarcheck` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
itarcheck ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
itarcheck ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
itarcheck ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
itarcheck ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `itarcheck` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="itarcheck") for rec in records]   # records = your JSON output
