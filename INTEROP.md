...
How **itarcheck** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `itarcheck` lives in the **Network / recon** cluster.
...
| from | relation | to |
|---|---|---|
| `itarcheck` | JSON findings compose with | [`nmapparse`](https://github.com/cognis-digital/nmapparse), [`pcapview`](https://github.com/cognis-digital/pcapview), [`dnsrecon`](https://github.com/cognis-digital/dnsrecon), [`subhunt`](https://github.com/cognis-digital/subhunt) |
| `itarcheck` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `itarcheck` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `itarcheck` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the network / recon cluster.** `itarcheck` output feeds the next tool:
```bash
itarcheck ... --format json > out.json          # this tool's findings (see `itarcheck --help`)
...
itarcheck ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
itarcheck ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
itarcheck ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `itarcheck`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `itarcheck` slots
