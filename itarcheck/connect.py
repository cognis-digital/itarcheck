"""Native cognis-connect emit for itarcheck — forward findings to any platform.

Maps itarcheck's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    itarcheck ... --format json | itarcheck-emit --to stix
    itarcheck-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('match', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('regime', '')
